...
public class PrintResultsReportFollowUpPosSelType {
    ...
    private static final String RESULTS_QUERY = "SELECT sample_id, surname, othernames, CONCAT(age, ' ', age_units) AS age, sex, referring_facility, address, reason_for_examination, results, request_date FROM request_list WHERE reason_for_examination = 'Follow Up' AND request_type = ? AND request_date BETWEEN ? AND ? AND results LIKE '%Scanty%' OR results LIKE '1+' OR results LIKE '2+' OR results LIKE '3+' OR results LIKE '4+'";
