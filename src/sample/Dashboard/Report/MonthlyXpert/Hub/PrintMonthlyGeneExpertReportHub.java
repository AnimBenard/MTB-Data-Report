...
public class PrintMonthlyGeneExpertReportHub {
    ...
    private static final String RESULTS_QUERY = "SELECT  'Total Test Performed' AS indicator, " +
            ...
            "FROM request_list WHERE MONTH(examined_date) = ? AND YEAR(examined_date) = ? AND results IS NOT NULL AND reason_for_examination IN ('TB Diagnosis', 'Drug Resistant') AND request_type = 'HUB' " +
            "UNION " +
            ...
            "FROM request_list WHERE MONTH(examined_date) = ? AND YEAR(examined_date) = ? AND results IS NOT NULL AND reason_for_examination = 'TB Diagnosis' AND request_type = 'HUB' " +
            "UNION " +
            ...
            "FROM request_list WHERE MONTH(examined_date) = ? AND YEAR(examined_date) = ? AND results IS NOT NULL AND reason_for_examination = 'Drug Resistant' AND request_type = 'HUB' " +
            "UNION " +
    ...
    XpertCartridgesMonthTableHub xpertCartridgesMonthTableHub = new XpertCartridgesMonthTableHub();
    ...
    public void addDataToTable(PdfPageBase page,String ward,  String year, String month) {
        ...
        xpertCartridgesMonthTableHub.addDataToTable2(page, (float) (result.getBounds().getHeight() + 50), "", year, month);
