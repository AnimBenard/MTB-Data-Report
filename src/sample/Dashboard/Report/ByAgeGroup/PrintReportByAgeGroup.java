package sample.Dashboard.Report.ByAgeGroup;
...
public class PrintReportByAgeGroup {
    ...
    private static final String RESULTS_QUERY = "SELECT " +
            ...
            "IFNULL(pos_count, 0) + IFNULL(neg_count, 0) AS total " +
            "FROM " +
            "(SELECT '0-10' AS age_group UNION ALL " +
            "SELECT '11-20' UNION ALL " +
            "SELECT '21-30' UNION ALL " +
            "SELECT '31-40' UNION ALL " +
            "SELECT '41-50' UNION ALL " +
            "SELECT '51-60' UNION ALL " +
            "SELECT '61-70' UNION ALL " +
            "SELECT '>70') AS age_ranges " +
            "LEFT JOIN " +
            "(SELECT " +
            "CASE " +
            ...
            "WHEN age > 70 THEN '>70' " +
            "END AS age_group, " +
            "SUM(CASE WHEN results = 'pos' THEN 1 ELSE 0 END) AS pos_count, " +
            "SUM(CASE WHEN results = 'neg' THEN 1 ELSE 0 END) AS neg_count " +
            "FROM " +
            "request_list WHERE MONTH(examined_date) = ? AND YEAR(examined_date) = ? " +
            "GROUP BY " +
            "age_group) AS result_counts " +
            "ON " +
            "age_ranges.age_group = result_counts.age_group";

    Connection conn = null;
    PreparedStatement preparedStatement = null;
    ResultSet resultSet = null;

    public void printReportForm(String year,String month, String datePrinted) {
        //margin
    ...
    public void savePDFReportForm(String year, String month, String datePrinted, String filePath) {
        //margin
        PdfDocument doc = new PdfDocument();
        PdfUnitConvertor unitCvtr = new PdfUnitConvertor();
        PdfMargins margin = new PdfMargins();
    ...
    public void addHeaderAndFooter(PdfDocument doc, Dimension2D pageSize, PdfMargins margin, String year, String month, String datePrinted) {
        ...
        String methodName = "Monthly Report";
        String wardName = "By Age Group";
    ...
    public void addDataToTable(PdfPageBase page, String year, String month) {
        ...
        table.getColumns().get(3).setWidth(width * 0.15f * width);
        table.getColumns().get(3).setStringFormat(new PdfStringFormat(PdfTextAlignment.Center, PdfVerticalAlignment.Middle));
        table.getStyle().setShowHeader(true);
        table.getColumns().get(0).setColumnName("Age Group");
        table.getColumns().get(1).setColumnName("Positive");
        table.getColumns().get(2).setColumnName("Negative");
        table.getColumns().get(3).setColumnName("Total");
