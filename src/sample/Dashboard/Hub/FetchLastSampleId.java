...
public class FetchLastSampleId {
    ...
    private static final String LAST_SERIAL_ID = "SELECT IFNULL(MAX(serial_id), 0) AS serialID FROM request_list WHERE YEAR(request_date) = YEAR(CURDATE()) AND request_type = 'HUB'";
    private static final String LAST_SERIAL_ID_SPOKES = "SELECT IFNULL(MAX(serial_id), 0) AS serialID FROM request_list WHERE YEAR(request_date) = YEAR(CURDATE()) AND request_type = 'SPOKES'";
